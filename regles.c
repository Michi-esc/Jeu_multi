...
static int mouvement_theorique_legal(Board *b, int x1, int y1, int x2, int y2) {
    ...
    switch (p.type) {
        case PAWN: {
            ...
            if (p.color == RED)    { dirY = -1; startRow = 10; } // Sud monte
            if (p.color == YELLOW) { dirY = 1;  startRow = 1;  } // Nord descend
            if (p.color == BLUE)   { dirX = 1;  startCol = 1;  } // Ouest va à droite
            if (p.color == GREEN)  { dirX = -1; startCol = 10; } // Est va à gauche
